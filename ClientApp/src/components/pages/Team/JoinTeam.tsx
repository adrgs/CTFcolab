...
@observer
export default class JoinTeam extends React.Component<any> {

    handleInviteCodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        this.props.TeamStore.values.inviteCode = e.target.value;
    };

    handleSubmitForm = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        this.props.TeamStore.joinTeam();
                        ...
                        <div className="p-6 mt-8">
                            <form action="#" onSubmit={this.handleSubmitForm}>
                                <div className="flex flex-col mb-2">
                                    <InputText name="team-invite-code" value={values.inviteCode} onChange={this.handleInviteCodeChange} type="text" placeholder="Team invite code" id="team-invite-code" />
                                </div>
                                <h2 className={'text-[#ED4A50] pb-4 ' + (errors ? 'visible' : 'invisible')}>
                                    Error: {errors}
                                </h2>
                                <div className="flex w-full my-4">
                                    <Button submit={true} label="Join Team" color="blue" disabled={inProgress} />
                                </div>
                            </form>
                        </div>
                    </div>
